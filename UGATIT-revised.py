...
class UGATIT(object) :
    ...
    def train(self):
        ...
        losses = torch.zeros(self.iteration, 3) # second col d loss, thrid col g loss
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            losses[step-1, 0] = step
            losses[step-1, 1] = Discriminator_loss
            losses[step-1, 2] = Generator_loss
        ...
        plt.plot(iterations.detach().numpy(), d_loss.detach().numpy())
        plt.xlabel('Iterations')
        plt.ylabel('D Losses')
        plt.title('D Losses VS Iterations')
        plt.savefig('DLossPlot.png')
        plt.clf()

        plt.plot(iterations.detach().numpy(), g_loss.detach().numpy(), label='g_loss')
        plt.xlabel('Iterations')
        plt.ylabel('G Losses')
        plt.title('G Losses VS Iterations')
        plt.savefig('GLossPlot.png')
        plt.clf()
    ...
    def test(self):
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            # B = RGB2BGR(tensor2numpy(denorm(fake_A2B2A[0])))

            # cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A', '%d.png' % (n + 1)), A)
            # cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B', '%d.png' % (n + 1)), B)
